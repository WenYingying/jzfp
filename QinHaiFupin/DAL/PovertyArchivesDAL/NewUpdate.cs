...
{
    public class NewUpdate : AbstractFactory.IFactory<PovertyArchives>
    {
        public bool Parameter(PovertyArchives _obj)
        {
            string sqltext = string.Format(@"UPDATE [dbo].[PovertyArchives] SET
                                          ...
                                          ,[EduLevels]='{8}'
                                          ,[StudentStatus]='{9}'
                                            WHERE ID={7}", _obj.Name.Replace("'", "''"), _obj.IdCardNo, _obj.Gender, _obj.BirthDay, _obj.Nation, _obj.HouseholderRelation.Replace("'", "''"), _obj.HouseholderId, _obj.ID, _obj.EduLevels.Replace("'", "''"), _obj.StudentStatus.Replace("'", "''"));
