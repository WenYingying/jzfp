...
{
    public class UpdatePovertyPropertiesByHolder : AbstractFactory.IFactory<Model.PovertyArchives>
    {
        public bool Parameter(PovertyArchives _obj)
        {
            string sqltext = string.Format(@"UPDATE [dbo].[PovertyArchives] SET
                                          [PovertyProperties]='{0}'
                                            WHERE HouseholderId={1}", _obj.PovertyProperties.Replace("'", "''"), _obj.HouseholderId);
