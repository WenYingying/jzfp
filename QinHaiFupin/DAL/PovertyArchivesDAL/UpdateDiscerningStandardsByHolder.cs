...
{
    public class UpdateDiscerningStandardsByHolder : AbstractFactory.IFactory<Model.PovertyArchives>
    {
        public bool Parameter(PovertyArchives _obj)
        {
            string sqltext = string.Format(@"UPDATE [dbo].[PovertyArchives] SET
                                          [DiscerningStandards]='{0}'
                                            WHERE HouseholderId={1}", _obj.DiscerningStandards.Replace("'", "''"), _obj.HouseholderId);
