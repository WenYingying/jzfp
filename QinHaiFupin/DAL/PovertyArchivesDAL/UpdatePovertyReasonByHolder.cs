...
{
    public class UpdatePovertyReasonByHolder : AbstractFactory.IFactory<Model.PovertyArchives>
    {
        public bool Parameter(PovertyArchives _obj)
        {
            string sqltext = string.Format(@"UPDATE [dbo].[PovertyArchives] SET
                                          [PovertyReason]='{0}'
                                            WHERE HouseholderId={1}", _obj.PovertyReason.Replace("'", "''"), _obj.HouseholderId);
